...
Simple staking (aka vanilla staking) is the most straightforward method for validators to opt-in to mev-commit and requires minimal setup. The vanilla registry allows validators to be opted-in directly by staking native ETH.

On the other hand, vanilla staking is not as capital efficient as using Symbiotic or Eigenlayer. As the ETH used to opt-in validators is completely separate from a validator's 32 ETH already staked with the beacon chain. Further, vanilla staking does not support the use of ERC20 collateral, and is not compatible with restaking in any way.

## Getting Started

Validators are able to _opt-in to mev-commit_ by _vanilla_ staking ETH directly with the `vanillaRegistry` contract.

This action confirms a validator’s attestation to follow the relay selection rule mentioned in the [Validator Guide](/v0.6.3/get-started/validators/validator-guide#relay-selection).
