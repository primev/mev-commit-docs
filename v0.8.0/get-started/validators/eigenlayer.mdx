...
If you plan to bulk opt-in a large number of validator pubkeys, check out the [Eigenlayer Operator Registration](/v0.8.0/knowledge-base/eigenlayer-operator-registration) guide.
...
Following operator delegation, each validator pubkey must be *registered* with the mev-commit AVS from the delegated operator's account, or from the pod owner account. This action confirms a validator's attestation to follow the relay selection rule mentioned in the [Validator Guide](/v0.8.0/get-started/validators/validator-guide#requirements).
