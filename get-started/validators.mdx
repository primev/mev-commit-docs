...
_Faucet page in progress_

Once you've obtained ether on the mev-commit chain, you can stake it on the validator registry page. Note the registry is designed such that one transaction can opt-in multiple validators accounts.

_Registry page in progress_

## Withdrawing staked ether

If you wish to withdraw your staked ether, you can do so on the previously mentioned validator registry page. Note an unstaking period is enforced, meaning you must first submit an `unstake` transaction, wait an appropriate amount of blocks, then submit a `withdraw` transaction, transferring funds to your account.
