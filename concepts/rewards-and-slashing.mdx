...
In addition to being able to stake directly on the mev-commit chain, validators have the option to restake their 32ETH staked on Ethereum’s beacon chain to opt-in to mev-commit. Currently, this restaking is supported only through the Eigenlayer protocol, but we expect to extend to more restaking protocols in the future. The staked tokens are used to secure the mev-commit protocol and to incentivize validators to include blocks built by mev-commit providers in the L1 chain. Tokens staked directly on the mev-commit chain through its validator registry contract can be withdrawn by the user upon request, while the restaked tokens may follow different rules depending on the restaking protocol. Crucially, if the user acts maliciously on the mev-commit protocol they can be slashed and lose a portion of their staked tokens. Next, we discuss the rewards and slashing mechanisms for providers and validators in more detail.
...
A validator that opts into mev-commit makes an implicit commitment to include a block from a mev-commit registered block builder in L1 whenever they become the proposer of the slot. Upon successfully including a block built by mev-commit providers in L1, the proposer is paid by the provider/builder through the mev-boost auction. An important observation here is that due to the preconf commitments, the total value of the block is expected to be higher than without preconfs. Thus, the proposer is expected to get a higher value for the block than just by using mev-boost alone.

### Proposer Rewards

1. a bidder sends a bid, and receives a commitment from a provider
2. The provider now has this additional bid amount to bid in the mev-boost auction, driving up the average mev-boost bid value that a proposer can get. mev-commit bids act similarly to net-new order flow for PBS actors, and increase potential revenue
3. The Proposer proposes a block that delivers the commitments the provider has made, and inherently receives the value through the mev-boost auction

Next, we dive into proposer slashing cases:
...
Case (1) is considered non-compliant behavior and thus the proposer will be slashed from their stake the fixed amount of 10 ETH. Case (2) should only happen very rarely when the value of the block it receives from the relay is too low, and mev-commit will not slash proposers in this case. However, due to the lack of a robust mechanism for builder attribution of a block in Ethereum's PBS, we will monitor how often proposers choose to build their own blocks, and if needed we will revisit this decision to discourage misbehavior by the proposers.
