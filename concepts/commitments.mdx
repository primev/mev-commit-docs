...
From the commitment provider generates digest, signs it with their private key, store digest and signature as encrypted commitment on the mev-commit chain, and sends the commitment back to the originating bidder via p2p network.
```json
{
   "result":{
