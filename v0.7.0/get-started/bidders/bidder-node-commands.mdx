...
<Note>Ensure you have completed the [Quickstart](/v0.7.0/get-started/quickstart) step in a separate terminal. This will set up your environment and place your key in the `$HOME/.mev-commit` directory, along with running the required bidder node.</Note>
...
To get the current deposit balance in the contract for the current [window](/v0.7.0/concepts/bids/bidder-deposit):

```shell ❯_ terminal
curl -s http://localhost:13523/v1/bidder/get_deposit
```

To get the current deposit balance in the contract for a specific [window](/v0.7.0/concepts/bids/bidder-deposit):
...
This command will allow you to withdraw funds from the specific [window](/v0.7.0/concepts/bids/bidder-deposit). 
