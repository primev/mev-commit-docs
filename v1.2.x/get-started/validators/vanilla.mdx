...
When you register validators, you send ETH as collateral **with the registration transaction**. That stake is **evenly distributed across all validators** included in that registration.
...
> **Clarification:** you can’t pre-fund future registrations. If you send more ETH than `minstake * n keys` during a registration, that extra ETH is simply spread across the validators registered in that tx and can be used as a slash buffer.
...
## Reputational VanillaRegistry

In addition to the canonical `VanillaRegistry`, we also operate a **reputational VanillaRegistry** instance.

The reputational registry is a separate contract instance. The functionality of this contract is the same, but has different `minStake` and slash settings.