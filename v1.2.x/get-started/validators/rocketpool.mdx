...
Registration can currently be done on Hoodi etherscan using the "Write as Proxy" tab on the [RocketMinipoolRegistry contract](https://hoodi.etherscan.io/address/0x30d478b02918c4b11731efc4868d848d551e79b2#writeProxyContract) (address {HoodiRocketMinipoolRegistryAddress}). Mainnet deployment is planned in the near future.
...
<div style="border:1px solid #e65100; padding:12px 16px; border-radius:10px; margin:16px 0;">
  <div style="font-weight:700; color:#e65100; margin-bottom:6px;">
    ⚠️ Avoid using your <strong>node address</strong> in a web browser
  </div>
  <div>
    For node security, it is best practice to avoid putting <strong>node address keys</strong> into web browsers due to security risks.
    When registering via a browser, use your <strong>withdrawal address</strong> instead,
    or perform the action locally via so keys never touch a website.
  </div>
</div>
