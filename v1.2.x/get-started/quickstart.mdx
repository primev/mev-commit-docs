...
<Steps>
  <Step title="(Optional) Create an account">
    ...
    If you already have a secure Ethereum account, you're good to skip to the next step. Otherwise there are many available options to create a new one:
    * Install foundry and create a keystore file using cast in a single command. The location of the generated file will be printed, otherwise you can specify a path argument according to [docs](https://getfoundry.sh/cast/reference/wallet/new/).
    ```shell
    ...
    * Install foundry and import an existing private key using cast in a single command. Further docs [here](https://getfoundry.sh/cast/reference/wallet/import).
    ...
    ```
    * [geth's clef tool](https://geth.ethereum.org/docs/fundamentals/account-management#creating-accounts)
    * Simply importing geth as a dependency in your golang project and programmatically creating a new keystore file. Read more [here](https://goethereumbook.org/keystore/).
    ```go
    package main
    import (
        "fmt"
  ...
  <Step title="Bridge to the mev-commit chain">
    ...
    Wait for the bridge transaction to complete. You can see detailed instructions in our [Bridge UI guide](/v1.2.x/get-started/bridge-ui).
    ...
    Or download the bridge CLI from a specific release (example links provided are for `1.2.0-rc3`) directly from the links below:
