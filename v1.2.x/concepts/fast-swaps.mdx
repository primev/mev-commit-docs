...
The swap flow is designed to feel like any familiar DEX — select tokens, enter an amount, review the quote, confirm. The key difference is the settlement layer: instead of the user submitting a swap transaction directly to a DEX router, Fast Swaps uses an intent-based architecture where a relayer executes the trade on the user's behalf.
...
Fast Swaps fetches real-time quotes from the **Uniswap V3 Quoter V2** contract on Ethereum mainnet. For each swap request, the system:

1. **Queries multiple fee tiers in parallel** — Uniswap V3 pools exist at 0.05%, 0.30%, and 1.00% fee tiers. Fast Swaps queries all three simultaneously and selects the tier that returns the best output amount.
...
Fast Swaps supports over **2,400 ERC-20 tokens** on Ethereum mainnet. Any token with liquidity on Uniswap V3 can be traded. The token list includes:
...
```
User                    Fast Swaps App              FAST RPC / Relayer        Ethereum L1
 │                          │                            │                       │
 │  1. Select tokens &      │                            │                       │
 │     enter amount         │                            │                       │
 │ ─────────────────────>   │                            │                       │
