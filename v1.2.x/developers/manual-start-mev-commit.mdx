...
<Steps titleSize="h3">
  ...
  <Step title="Get the mev-commit binary">
    ...
    curl -L \
    "https://github.com/primev/mev-commit/releases/download/v1.2.x/$FILENAME" \
    | tar -xz -C $HOME
    ```
    ...
    <Steps>
      ...
      <Step title="Grab the checksum">
        ...
        CHECKSUM=$(curl -L \
        "https://github.com/primev/mev-commit/releases/download/v1.2.x/mev-commit_v1.1.0_checksums.txt" \
...
<CardGroup cols={2}>
  <div className="inline-card-header">
    <Card title="Bidders" icon="user-bounty-hunter">
      ...
      process](/v1.2.x/developers/bidder-deposit) to start sending bids.
    </Card>
  </div>
  <div className="inline-card-header">
    <Card title="Providers" icon="network-wired">
      ...
      process](/v1.2.x/get-started/providers/registering-a-provider).
    </Card>
  </div>
</CardGroup>
