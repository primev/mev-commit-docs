...
<Steps titleSize="h3">
  ...
  <Step title="Get the mev-commit binary">
    ...
    curl -L \
    "https://github.com/primev/mev-commit/releases/download/v0.8.0/$FILENAME" \
    | tar -xz -C $HOME
    ```
    ...
    <Steps>
      ...
      <Step title="Grab the checksum">
        ...
        FILENAME="mev-commit_$(uname -s)_$(uname -m).tar.gz"
        CHECKSUM=$(curl -L \
        "https://github.com/primev/mev-commit/releases/download/v0.8.0/mev-commit_v0.8.0_checksums.txt" \
...
<CardGroup cols={2}>
  <div className="inline-card-header">
    <Card title="Bidders" icon="user-bounty-hunter">
      ...
      process](/v0.8.0/developers/bidder-deposit) to start sending bids.
    </Card>
  </div>
  <div className="inline-card-header">
    <Card title="Providers" icon="network-wired">
      ...
      process](/v0.8.0/get-started/providers/registering-a-provider).
    </Card>
  </div>
</CardGroup>
